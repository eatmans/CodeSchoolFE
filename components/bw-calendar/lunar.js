...
const Lunar = {
	...
	toGanZhiYear(lYear) {
		...
		return Lunar.Gan[ganKey - 1] + Lunar.Zhi[zhiKey - 1]
	},
	...
	solar2lunar(y, m, d) { // 参数区间1900.1.31~2100.12.31
		...
		// 天干地支处理
		var sm = m - 1
		var gzY = Lunar.toGanZhiYear(year)
	...
	lunar2solar(y, m, d, isLeapMonth) { // 参数区间1900.1.31~2100.12.1
		...
		var leapOffset = 0
		var leapMonth = Lunar.leapMonth(y)
		var leapDay = Lunar.leapDays(y)
		...
		var day = Lunar.monthDays(y, m)
		var _day = day
		// bugFix 2016-9-25
		// if month is leap, _day use leapDays method
		if (isLeapMonth) {
			_day = Lunar.leapDays(y, m)
		}
		...
		for (var i = 1900; i < y; i++) {
			offset += Lunar.lYearDays(i)
		}
		var leap = 0;
		var isAdd = false
		for (var i = 1; i < m; i++) {
			leap = Lunar.leapMonth(y)
			if (!isAdd) { // 处理闰月
				if (leap <= i && leap > 0) {
		...
		return Lunar.solar2lunar(cY, cM, cD)
	}
}
